...
class FocalLoss(nn.Module):
    ...
    def __call__(self, input, target):
        bceloss = F.binary_cross_entropy(input, target, reduction='none')
...
train_dataset = DMRDataset(os.path.join(conf.data), conf.train_fold, train=True)
...
model = unet3d.UNet3D(in_channel=1, out_channel=6)
...
for epoch in range(intial_epoch,conf.nb_epoch):
	...
	for i in range(5):
		...
		if True:
			...
			sample_1 = np.concatenate((x[0,2*x.shape[1]//6,:,:], y[5,2*x.shape[1]//6,:,:], p[5,2*x.shape[1]//6,:,:]), axis=0)
			sample_2 = np.concatenate((x[0,3*x.shape[1]//6,:,:], y[5,3*x.shape[1]//6,:,:], p[5,3*x.shape[1]//6,:,:]), axis=0)
			sample_3 = np.concatenate((x[0,4*x.shape[1]//6,:,:], y[5,4*x.shape[1]//6,:,:], p[5,4*x.shape[1]//6,:,:]), axis=0)
			sample_4 = np.concatenate((x[0,5*x.shape[1]//6,:,:], y[5,5*x.shape[1]//6,:,:], p[5,5*x.shape[1]//6,:,:]), axis=0)
